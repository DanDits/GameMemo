...
public final class GameKey {
	...
	// If for grammar reasons the name is not fitting (for example it is not an eigenname), use this as in GameSetupActivity.
	public static CharSequence getGameNameSingular(int gameKey, Resources res) {
	    switch (gameKey) {
	    case CUSTOMGAME:
            return res == null ? CustomGame.NAME : res.getString(R.string.user_game_name_singular);
	    default:
	        return getGameName(gameKey, res);
	    }
	}
	...
	public static void applyTheme(int gameKey, Resources res, View view) {
	    ...
	    if (view instanceof TextView) {
	        applyThemeTextColor(gameKey, res, (TextView) view);
	    }
	}
	
	public static void applyThemeTextColor(int gameKey, Resources res, TextView view) {
        view.setTextColor(res.getColorStateList(getButtonTextColorResource(gameKey)));	    
	}
