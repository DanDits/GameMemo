...
public class DoppelkopfGameStatisticAttributeManager extends
        GameStatisticAttributeManager {
    public static final DoppelkopfGameStatisticAttributeManager INSTANCE = new DoppelkopfGameStatisticAttributeManager();

    public DoppelkopfGameStatisticAttributeManager() {
        super(GameKey.DOPPELKOPF);
    }

    @Override
    protected boolean containsAllTeams(Game pGame, AttributeData data) {
        DoppelkopfGame game = (DoppelkopfGame) pGame;
    ...
    public Bundle createTeamsParameters(int mode, Resources res,
        ...
        if (mode == StatisticsActivity.STATISTICS_MODE_ALL) {
            builder.addTeam(DoppelkopfGame.MIN_PLAYERS, DoppelkopfGame.MAX_PLAYERS, false, defaultTeamName, false, TeamSetupViewController.DEFAULT_TEAM_COLOR, true, teamNamesToArray(teamSuggestions, 0, teams));
        } else {
            for (int i = 0; i < DEFAULT_STATISTIC_MAX_TEAMS; i++) {
                builder.addTeam(DoppelkopfGame.MIN_PLAYERS, DoppelkopfGame.MAX_PLAYERS, i >= 1, defaultTeamName, false, TeamSetupViewController.DEFAULT_TEAM_COLOR , true, teamNamesToArray(teamSuggestions, i, teams));
            }
        }
