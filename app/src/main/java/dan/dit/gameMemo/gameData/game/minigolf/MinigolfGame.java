...
public class MinigolfGame extends SportGame {
    ...
    public String getFormattedInfo(Resources res) {
        ...
        int index = 0;
        for (Player p : mPlayers) {
            builder.append(p.getName()).append(" (").append(getScore(index)).append(")\n");
            index++;
        }
    ...
    public boolean removePlayer(int playerIndex) {
        if (mPlayers.size() > 1 && playerIndex >= 0 && playerIndex < mPlayers.size()) {
            mPlayers.remove(playerIndex);
            for (GameRound r : rounds) {
                MinigolfRound round = (MinigolfRound) r;
                round.removePlayer(playerIndex);
            }
            return true;
        }
        return false;
