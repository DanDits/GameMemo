...
import dan.dit.gameMemo.appCore.SportGameDetailFragment;
...
public class MinigolfGameDetailFragment extends SportGameDetailFragment {
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        inflater.inflate(R.menu.minigolf_game_detail, menu);
    }
    ...
    private void adaptInputListeners() {
        ...
        mInputController.addListener(new OnOperationClickedListener() {
            ...
            @Override
            public void onOperationClickedBeforeExecute(Operation which) {}
            
            @Override
            public void onOperationClickedAfterExecute(Operation which) {
                if (which instanceof AlgebraicOperation || which instanceof SignOperation || which instanceof AbsoluteOperation) {
                    selectPlayer(((MinigolfRound) mGame.getRound(mSelectedBahn)).getFirstIncompletePlayer()); 
                }
            }
        });
                        ...
                        if (op instanceof AlgebraicOperation || op instanceof AbsoluteOperation || op instanceof SignOperation) {
                            ...
                            if (key.equals(CUSTOM_OPERATION_PREV_BAHN_KEY)) {
                                if (isInverse) {
                                    ...
                                    int roundIndex = mSelectedBahn > 0 ? (mSelectedBahn - 1) : (mGame.getRoundCount() - 1);
                                    selectRound(roundIndex); // cycle through - continue at the end when it reached the first bahn
                                }
    ...
    private void initInput() {
        if (mGame != null) {
            mInputController = new NumberInputController(GameKey.MINIGOLF, getActivity(), mInputContainer, false, null);
    ...
    private void onPrevBahnClick() {
        if (mGame.getRoundCount() > 1) {
            ...
            selectPlayer(mGame.getFirstIncompletePlayer(mSelectedBahn));
        }
    }
    ...
    private void loadGame(long gameId, Bundle saveInstanceState) {
        ...
        if (games != null && games.size() > 0) {
            ...
            selectPlayerExecute(mGame.getFirstIncompletePlayer(mSelectedBahn));
        } else {
    ...
    private void selectPlayerExecute(int playerPos) {
        ...
        fillData();
        mScoresView.smoothScrollToPosition(mSelectedPlayer);
    }
    ...
    private void fillData() {
        ...
        if (mSelectedBahn == mGame.getRoundCount() - 1) {
            // selected last bahn
            mNextBahn.setImageResource(R.drawable.plus_small);
        } else {
        ...
        if (mAdapter == null) {
            ...
            mScoresView.setOnItemLongClickListener(new OnItemLongClickListener() {
                ...
                public boolean onItemLongClick(AdapterView<?> parent, View v,
                    ...
                    DialogFragment dialog = ChoosePlayerDialogFragment.newInstance(position, null, false, mGame.getPlayerCount() > 1);
    ...
    public void playerRemoved(int arg, Player removed) {
        if (removed != null && removed.equals(mGame.getPlayers().get(arg))) {
            if (mGame.removePlayer(arg)) {
                selectPlayerExecute(0);
