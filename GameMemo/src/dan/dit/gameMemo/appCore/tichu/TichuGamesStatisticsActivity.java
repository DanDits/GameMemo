 ...
 * where statistics are visualized for a player that can be selected from a spinner
 * and one tab where a statistic of a certain type is visualized for all players.<br>
 * Loading games and building statistics is done in a background thread and will not block the UI.
 * @author Daniel
 *
 */
public class TichuGamesStatisticsActivity extends Activity {
	...
	//TODO this activity needs to be reworked, offering a more general approach to visualize and handle statistics, making it easier and more flexible to add new ones
	@SuppressLint("NewApi")
	@Override
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			mOnlyShowSignificantForGenre = savedInstanceState.getBoolean(STORAGE_ONLY_SHOW_SIGNIFICANT, DEFAULT_ONLY_SHOW_SIGNIFICANT);
			mSortByPercentage = savedInstanceState.getBoolean(STORAGE_SORT_BY_PERCENTAGE, DEFAULT_SORT_BY_PERCENTAGE);
		}
		...
		spec1.setIndicator(getResources().getString(R.string.statistics_by_player));
		TabHost.TabSpec spec2 = tabs.newTabSpec("tag2");
		spec2.setContent(R.id.statistics_b_genre);
		spec2.setIndicator(getResources().getString(R.string.statistics_by_genre));
		tabs.addTab(spec1);
		tabs.addTab(spec2);
		// init stats
		refreshStatistics();
	...
	private void refreshByPlayerTab() {
		ArrayAdapter<Player> playerAdapter = new ArrayAdapter<Player>(this, android.R.layout.simple_spinner_item, android.R.id.text1);
	...
	private void refreshByGenreTab() {
		ArrayAdapter<TichuStatisticType> typeAdapter = new ArrayAdapter<TichuStatisticType>(this, 
	...
	private void loadAndBuildStats() {
		this.statBuilder = new AsyncLoadAndBuildStatistics(getContentResolver(), players, GameKey.TICHU);
		...
		this.statBuilder.execute(GameStorageHelper.getUriAllItems(GameKey.TICHU));
	}
	
	private void clearPlayerStatistics() {
		((TextView) findViewById(R.id.statistic_average_finisher_pos)).setText("");
		...
		if (stat != null) {
			...
			if (index >= 0) {
				...
				((TextView) findViewById(R.id.statistics_score_per_round_incl_tichus))
					.setText(dataToString(data[TichuStatisticType.SCORE_PER_ROUND_INCL_TICHUS.ordinal()]));
				((TextView) findViewById(R.id.statistics_score_per_round_no_tichus))
					.setText(dataToString(data[TichuStatisticType.SCORE_PER_ROUND_NO_TICHUS.ordinal()]));
				((TextView) findViewById(R.id.statistic_average_finisher_pos))
					.setText(dataToString(data[TichuStatisticType.FINISHER_POS_AVERAGE.ordinal()]));
				fillRow_TotalWonPercentage((TextView) findViewById(R.id.statistic_big_tichu_bids),
						(TextView) findViewById(R.id.statistic_big_tichus_won), 
						(int) data[TichuStatisticType.BIG_TICHU_BIDS_ABS.ordinal()],
						(int) data[TichuStatisticType.BIG_TICHUS_WON_ABS.ordinal()]);
				fillRow_TotalWonPercentage((TextView) findViewById(R.id.statistic_small_tichu_bids),
						(TextView) findViewById(R.id.statistic_small_tichus_won), 
						(int) data[TichuStatisticType.SMALL_TICHU_BIDS_ABS.ordinal()],
						(int) data[TichuStatisticType.SMALL_TICHUS_WON_ABS.ordinal()]);
				fillRow_TotalWonPercentage((TextView) findViewById(R.id.statistic_games_played),
				...
				fillRow_TotalWonPercentage((TextView) findViewById(R.id.statistic_rounds_played),
						(TextView) findViewById(R.id.statistic_rounds_won), 
						(int) data[TichuStatisticType.GAME_ROUNDS_PLAYED.ordinal()],
						(int) data[TichuStatisticType.GAME_ROUNDS_WON_RAWSCORE_ABS.ordinal()]);
			} else {
				clearPlayerStatistics();
			}
		} else {
	...
	private void fillGenresStatistics() {
		...
		if (stat != null) {
			...
			TichuStatisticType selectedType = selItem == null ? null : (TichuStatisticType) selItem;
			if (selectedType != null) {
				double[] data = stat.getStatistic(selectedType);
				boolean[] dataIncluded = new boolean[data.length];
				// selection sort, since i need the permutation indices which are not supplied by Collections.sort, there will never be too many players anyway
				boolean done = false;
				TableLayout table = ((TableLayout) findViewById(R.id.statistics_by_genre_table));
				while (!done) {
					done = true;
					...
					for (int i = 0; i < data.length; i++) {
						double currData;
						if (mSortByPercentage && selectedType.getAbsoluteType() != null) {
							int absData = (int) stat.getStatistic(selectedType.getAbsoluteType(), i);
							currData = absData == 0 ? (-Double.MAX_VALUE) : data[i] / absData;
						} else {
							currData = data[i];
						}
					...
					if (highestIndex >= 0) {
						// found a valid player index, check if this player data should be shown
						dataIncluded[highestIndex] = true;
						boolean isDataMeaningful = selectedType.isMeaningfulData(max, 
								stat.getStatistic(TichuStatisticType.GAME_ROUNDS_PLAYED, highestIndex)
								, stat.getStatistic(TichuStatisticType.GAMES_PLAYED, highestIndex));
						if (!mOnlyShowSignificantForGenre || isDataMeaningful) {
							// showing every player data or player data is significant as defined by the data type
							TableRow row = (TableRow) getLayoutInflater().inflate(R.layout.statistics_by_genre_table_row, table, false);
							TextView playerNameView = ((TextView) row.findViewById(R.id.statistics_by_genre_row_player_name));
							TextView valueView = ((TextView) row.findViewById(R.id.statistics_by_genre_row_value));
							if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
								if (isDataMeaningful) {
									playerNameView.setAlpha(1f);
									valueView.setAlpha(1f);
								} else {
									playerNameView.setAlpha(DEFAULT_SIGNIFICANT_BUT_ANYWAYS_SHOWN_ALPHA);
									valueView.setAlpha(DEFAULT_SIGNIFICANT_BUT_ANYWAYS_SHOWN_ALPHA);
								}
							}
							playerNameView.setText(players.get(highestIndex).getName());
							valueView.setText(dataToString(data[highestIndex], 
									selectedType.getAbsoluteType() != null ? (int) stat.getStatistic(selectedType.getAbsoluteType(), highestIndex) : Integer.MIN_VALUE));
							if (selectedType.isHigherBetter()) {
								table.addView(row); // add to end of table
							} else {
								table.addView(row, 1); // add to front, at 0.row there is the header
							}
						}
					}
				}
	...
	private void fillRow_TotalWonPercentage(TextView totalValueView, TextView wonValueView, int total, int won) {
	...
	private String dataToString(double data, int baseData) {
		if (baseData != Integer.MIN_VALUE && baseData != 0) {
			...
			if (DEFAULT_SHOW_WON_PERCENTAGE) {
				builder.append(' ');
				builder.append('(');
				builder.append(NumberFormat.getPercentInstance().format(data / (double) baseData));
				builder.append(')');
			}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		menu.findItem(R.id.statistics_toggle_sort_type).setTitle(getIdOfNextSortType());
		menu.findItem(R.id.statistics_only_show_significant).setChecked(mOnlyShowSignificantForGenre);
		return true;
	}
	...
	private void setTichuStatisticTypeName(TichuStatisticType type) {
		...
		case BIG_TICHU_BIDS_ABS:
			type.setName(getResources().getString(R.string.statistics_big_tichus_descr));
			break;
		case BIG_TICHUS_WON_ABS:
			type.setName(getResources().getString(R.string.statistics_big_tichus_won_descr));
			break;
		case FINISHER_POS_AVERAGE:
			type.setName(getResources().getString(R.string.statistics_average_finisher_pos_descr));
			break;
		case GAME_ROUNDS_PLAYED:
			type.setName(getResources().getString(R.string.statistics_rounds_played_descr));
			break;
		case GAME_ROUNDS_WON_RAWSCORE_ABS:
			type.setName(getResources().getString(R.string.statistics_rounds_won_descr));
			break;
		case GAMES_PLAYED:
			type.setName(getResources().getString(R.string.statistics_games_played_descr));
			break;
		case GAMES_WON_ABS:
			type.setName(getResources().getString(R.string.statistics_games_won_descr));
			break;
		case SMALL_TICHU_BIDS_ABS:
			type.setName(getResources().getString(R.string.statistics_small_tichus_descr));
			break;
		case SMALL_TICHUS_WON_ABS:
			type.setName(getResources().getString(R.string.statistics_small_tichus_won_descr));
			break;
		case SCORE_PER_ROUND_INCL_TICHUS:
			type.setName(getResources().getString(R.string.statistics_score_per_round_incl_tichus));
			break;
		case SCORE_PER_ROUND_NO_TICHUS:
			type.setName(getResources().getString(R.string.statistics_score_per_round_no_tichus));
			break;
		}
	}
	...
	public void onSaveInstanceState(Bundle outState) {
		...
		outState.putBoolean(STORAGE_ONLY_SHOW_SIGNIFICANT, mOnlyShowSignificantForGenre);
